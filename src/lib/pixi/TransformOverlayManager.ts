...
function updateCropGrid(boundsBox: PIXI.Graphics, width: number, height: number, isDark: boolean, overlayColor: number) {
  boundsBox.clear();
  ...
  boundsBox.stroke({ width: 1, color: isDark ? 0xffffff : 0x111111, alpha: 0.6 });
}
...
export class TransformOverlayManager {
  ...
  public update(activeLayerId: string | null, theme: string, zoom: number, activeTool: string) {
    ...
    const layer = state.layers.find((l) => l.id === activeLayerId) as ImageLayer | undefined;
    ...
    if (isCropMode) {
      if (layer.cropRect && layer.originalWidth > 0) {
        ...
        offsetX = (layer.cropRect.x - (layer.originalWidth - layer.cropRect.width) / 2) * Math.abs(layer.scaleX);
        offsetY = (layer.cropRect.y - (layer.originalHeight - layer.cropRect.height) / 2) * Math.abs(layer.scaleY);
      }
    }
    ...
    this.border.clear();
    if (!isCropMode) {
      this.border.rect(-scaledWidth / 2, -scaledHeight / 2, scaledWidth, scaledHeight);
      this.border.stroke({ width: 1.5, color: overlayColor, alpha: overlayAlpha });
    }
    ...
    if (isCropMode) {
      updateCropGrid(this.cropGrid, scaledWidth, scaledHeight, isDark, overlayColor);
    ...
    for (const pos of expectedHandles) {
      if (!this.handles[pos.id]) {
        ...
        drawHandle(handle, pos, isCropMode, handleFillColor, handleStrokeColor, handleStrokeAlpha, overlayColor);
        this.handles[pos.id] = handle;
        this.container.addChild(handle);
      } else {
        this.handles[pos.id].clear();
        drawHandle(this.handles[pos.id], pos, isCropMode, handleFillColor, handleStrokeColor, handleStrokeAlpha, overlayColor);
      }
      ...
      handle.x = stagePos.x + unrotatedX * Math.cos(angle) - unrotatedY * Math.sin(angle);
      handle.y = stagePos.y + unrotatedX * Math.sin(angle) + unrotatedY * Math.cos(angle);
