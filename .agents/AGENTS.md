...
- **Smart Crop & Bounds Engine**: Never destructively replace Image Blobs to crop transparent backgrounds. AI/Smart Crop workers MUST return bounding box coordinates (`minX`, `minY`, `maxX`, `maxY`). The React layer (`useSmartCrop`) then updates the layer's `cropRect` and applies geometric center-shifts (`globalShiftX`, `globalShiftY`) to perfectly synchronize with the dynamic crop frame in the `TransformOverlayManager`.
...
### 13. Decoupled Canvas Architecture

- **Registry-Based Surface Routing**: The `WorkspaceLayout` MUST conditionally mount canvases (like `ImageCanvas` or `PdfCanvas`) based strictly on the `surfaceType` metadata defined in `src/lib/toolRegistry.ts`.
- **Canvas Agnosticism**: Canvas components must remain "dumb." They should never import specific tool logic. They blindly render the Zustand state (`useLayerStore`) and rely on universal `interactionModes` (like `'crop'`, `'pan'`).

### 14. Smart Export Engine

- **Unified Strategy Interface**: All new export formats must implement the `IExportEngine` interface in `src/engines/types.ts`.
- **Dumb Modal Shell**: The `<ExportModal>` component is strictly a shell. It must defer to `exportManager.getActiveEngine().getUI()` to render all internal UI and handle export execution, ensuring the modal remains decoupled from specific formats like UPNG or pdf-lib.

### 15. PDF Architecture (Normalized Flat State & Native DOM)

- **State Engine**: Multi-page documents MUST be modeled using a **Normalized Flat State**. Keep `layers` as a flat array in `useLayerStore`, but add a `parentId` to associate them with specific pages. Pages themselves are just layers of type `'page'`. Do NOT build deeply nested `pages: { layers: [] }[]` arrays.
- **Rendering Engine**: Do NOT use PixiJS for PDFs. The `PdfCanvasArea` MUST use a bespoke Native DOM/Canvas approach powered by `pdf.js` to ensure text remains crisp and selectable.

### 16. Web Worker & AI Pipeline Orchestration

- **Tool-Mount Preloading**: When a user mounts an AI or Video tool page, the `WorkerManager` MUST immediately spin up the background worker and begin loading heavy models into VRAM. Do NOT wait for the user to click "Apply" to instantiate the worker, as this degrades UX with long loading times.

### 17. Dynamic Properties Panel State

- **Global Tool State**: Live slider and toggle values generated by the `DynamicPropertiesPanel` MUST be stored globally in `useToolStore` (e.g., `toolParams: Record<string, any>`). Do NOT use local React `useState` for these parameters, as Canvas overlays (like crop bounding boxes) need real-time synced access to the exact same values.

### 18. Dynamic Transform Overlays & Canvas Interactions

- **Do Not Hardcode Overlay Logic**: When adding interactive bounding boxes, resize handles, or rotation handles to a tool, NEVER hardcode the tool's ID inside `TransformOverlayManager.ts`.
- **Use toolRegistry.ts**: Instead, export `showTransformOverlay: true` and/or `allowRotation: false` in the tool's `ToolDefinition`. The canvas manager will dynamically read these flags and render the GPU-accelerated handles automatically.
- **Gesture Architecture**: Handle dragging (scaling/rotating) and canvas panning/zooming are intercepting `globalpointermove` natively on the PixiJS stage. Do not attempt to manage these continuous transform gestures using React state, as it will cause severe performance degradation.
