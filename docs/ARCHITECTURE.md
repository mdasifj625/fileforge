...
- **Hardware Acceleration & Fallback**: The inference engine implements a robust cascade (`WebGPU` -> `WebNN-NPU` -> `WebNN` -> `WASM`). This guarantees that modern smartphones and Apple Silicon leverage their Neural Processing Units (NPUs) or GPUs for lightning-fast speeds, while older devices seamlessly fall back to multi-threaded CPU execution without breaking.
- **Dynamic Scaling & Profiling**: Automatically downscales high-megapixel images to optimal tensor boundaries (`512px`, `1024px`) before inference to prevent WebGL memory allocation crashes, then intelligently upscales the generated alpha mask back to native resolution. A `PerformanceProfiler` captures exact telemetry step-by-step.
